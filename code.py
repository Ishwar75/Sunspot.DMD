...
def real_vector(vector):
    return list(map(lambda x: -1.0 * x.real, vector))


# return imaginary part of a vector
def imag_vector(vector):
    return list(map(lambda x: x.imag, vector))
...
def on_pick(event):
    ...
    pylab.plot(numpy.linspace(90, -90, num=50), numpy.real(dmd_m[:, index]),
               label="Real part")  # , numpy.linspace(0,49,num=50)
...
mat_contents = sio.loadmat(r'C:\Users\ASUS\Downloads\sunspot_data.mat')  # MatLab file of butterfly diagram data
# Data source: https://solarscience.msfc.nasa.gov/greenwch/bflydata.txt

# If signal falls to zero, maybe smoothing will help:
X = scipy.ndimage.filters.gaussian_filter(mat_contents['data_v'], 2)
...
# Display eigenvalues closest in magnitude to one:
disp_num = 11
...
exp_theta = list(map(lambda x: exp(1j * x), theta))
...
for i in range(n_modes):
    for j in range(mode_count):
        ...
        quad_mat[i, j] = numpy.linalg.norm(mode)  # magnitude of countth mode of d
        iter_count += 1
    mode_count -= 1
...
pylab.xlabel('POD mode j')
pylab.ylabel('POD mode i')
pylab.show()
